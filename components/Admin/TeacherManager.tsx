...
const TeacherManager: React.FC<TeacherManagerProps> = ({ onNotify }) => {
  ...
  return (
    <div className="p-8 animate-fade-in font-[Roboto]">
      <div className="bg-slate-50 border border-slate-200 p-6 rounded-sm mb-8 flex flex-col md:flex-row items-start md:items-center gap-6 shadow-sm">
         ...
         <div className="flex gap-3">
            <button onClick={() => setShowAddModal(true)} className="px-6 py-3 bg-blue-600 text-white rounded-sm font-black text-[10px] uppercase tracking-widest hover:bg-blue-700 transition-all flex items-center gap-2 shadow-lg">
                <i className="fas fa-plus"></i> Thêm Cán bộ quản lý mới
            </button>
            <button onClick={fetchProfiles} className="px-6 py-3 bg-white border border-slate-200 text-slate-700 rounded-sm font-black text-[10px] uppercase tracking-widest hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-all flex items-center gap-2 shadow-sm shrink-0">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-6 border-b border-slate-100 pb-6">
        ...
        <div className="flex bg-slate-100 p-1 rounded-sm border border-slate-200">
            <button onClick={() => setActiveTab('TEACHERS')} className={`px-6 py-2 rounded-sm text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === 'TEACHERS' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:bg-white'}`}><i className="fas fa-user-tie"></i> Cán bộ</button>
            <button onClick={() => setActiveTab('CANDIDATES')} className={`px-6 py-2 rounded-sm text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeTab === 'CANDIDATES' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:bg-white'}`}><i className="fas fa-users"></i> Học viên</button>
        </div>
      </div>
          ...
          <div className="bg-white w-full max-w-md rounded-sm p-8 shadow-2xl animate-slide-up border-t-4 border-blue-600">
            ...
            <div className="space-y-5">
              <div className="space-y-1">
                ...
                <input type="text" value={newTeacherName} onChange={e => setNewTeacherName(e.target.value)} className="w-full p-3 bg-slate-50 border-2 border-slate-200 rounded-sm outline-none focus:border-blue-600 focus:bg-white font-bold text-sm text-slate-800 transition-all" autoFocus placeholder="Ví dụ: Nguyễn Văn A" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black text-blue-600 uppercase tracking-widest ml-1">Email Đăng nhập</label>
                <input type="email" value={newTeacherEmail} onChange={e => setNewTeacherEmail(e.target.value)} className="w-full p-3 bg-slate-50 border-2 border-slate-200 rounded-sm outline-none focus:border-blue-600 focus:bg-white font-bold text-sm text-slate-800 transition-all" placeholder="teacher@domain.edu.vn" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black text-blue-600 uppercase tracking-widest ml-1">Mật khẩu Khởi tạo</label>
                <input type="password" value={newTeacherPassword} onChange={e => setNewTeacherPassword(e.target.value)} className="w-full p-3 bg-slate-50 border-2 border-slate-200 rounded-sm outline-none focus:border-blue-600 focus:bg-white font-bold text-sm text-slate-800 transition-all" placeholder="Tối thiểu 8 ký tự" />
              </div>
              <div className="flex gap-3 pt-4 border-t border-slate-100 mt-6">
                <button type="button" onClick={() => setShowAddModal(false)} className="flex-1 py-3 bg-slate-100 text-slate-500 rounded-sm font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all">Hủy</button>
                <button onClick={handleAddTeacher} disabled={isCreating} className="flex-1 py-3 bg-blue-600 text-white rounded-sm font-black text-[10px] uppercase tracking-widest shadow-lg hover:bg-blue-700 transition-all disabled:opacity-70">{isCreating ? 'Đang tạo...' : 'Tạo tài khoản'}</button>
              </div>
            </div>
          </div>
        </div>, document.body
